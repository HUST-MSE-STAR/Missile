...
uint8_t s1c=0,s2c=0;	//只是两个三位开关是否发生变化的变量,为0表示无变化，1表示有变化
...
static void sbus_to_rc(volatile const uint8_t *sbus_buf, RC_ctrl_t *rc_ctrl)
{
    ...
    rc_ctrl->rc.ch[3] = ((sbus_buf[4] >> 1) | (sbus_buf[5] << 7)) & 0x07ff; //!< Channel 3
		...
		if(rc_ctrl->rc.s[0] != ((sbus_buf[5] >> 4) & 0x0003)){
		s1c=1;
    rc_ctrl->rc.s[0] = ((sbus_buf[5] >> 4) & 0x0003) >> 2;                //!< Switch left
    }   
		else {
		s1c=0;
		}
		
		if(rc_ctrl->rc.s[1] != (((sbus_buf[5] >> 4) & 0x000C) >> 2)){
		s2c=1;
